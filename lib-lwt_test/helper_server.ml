     ...
     fun _client_address { Gluten.reqd; _ } ->
      ...
      let response =
        Response.create
          ~headers:
            (Headers.of_list
               [ ( Piaf_lwt.Headers.Well_known.content_length
              ...
              let ret =
                ...
                | Some ssl_socket ->
                  ...
                  | None (* Unable to negotiate a protocol *) | Some _ ->
                    ...
                    assert false)
              in
              if Lwt.is_sleeping error_p
              then Lwt.wakeup_later wakeup_error (Ok ());
              ret)
            (fun exn ->
              Format.eprintf "EXN: %s@." (Printexc.to_string exn);
              if Lwt.is_sleeping error_p
              then Lwt.wrap2 Lwt.wakeup_later wakeup_error (Error (`Exn exn))
